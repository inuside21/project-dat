...
// Entry Point
// ===============================================
export default function ProfileEquipmentReturnData({ navigation, route }) {
  // Variable
  // ===========================
  ...
  const OnPressReceived = () => {
    ...
    const urlReq = ConfigConnection.server + "api.php?mode=122";
    fetch(urlReq, {
      method: 'POST',
      headers: { 'Content-Type': 'multipart/form-data' },
      body: JSON.stringify({
              ...
              <View style={styles.mainForm} >
                ...
                <Surface elevation={2} style={{ marginVertical: 10, height: 700 }}>
                  ...
                  <View style={{ flex: 1, marginHorizontal: 70 }}>
                    { transactionData.trans_return == "0" &&
                        <View style={{ flexDirection: "row" }}>
                          ...
                          <Button mode="contained" buttonColor={ConfigColors.GREENCOLOR} contentStyle={{ height: 40 }} onPress={OnPressReceived} style={styles.formButton}>
                            Returned
                          </Button>
                        </View>
                    }
                    { transactionData.trans_return == "1" &&
                        <View>
                          <Button mode="contained" buttonColor={ConfigColors.GREENCOLOR} contentStyle={{ height: 40 }}>
                            Received Product! 
                          </Button>
                        </View>
                    }
