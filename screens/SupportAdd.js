...
// Entry Point
// ===============================================
export default function SupportAdd({ navigation, route }) {
  // Variable
  // ===========================
  ...
  // Start
  {
    ...
    useEffect(() => {
      if (selectMode != "")
      {
        navigation.navigate("SelectCategory", { sdata: selectMode, slink: "SupportAdd" })
        setSelectMode("")
      }
    }, [selectMode])
  ...
  const onPressSubmit = () => {
    ...
    // send data
    {
      const urlReq = ConfigConnection.server + "api.php?mode=40";
      fetch(urlReq, {
        method: 'POST',
      ...
      .then((dat) => {
        if (dat.status == "ok")
        {
          navigation.navigate('ViewSupport', { data: userData, sdata: formCat, smode: 43 })
        }
        else
        {
                ...
                <Surface elevation={2} style={{ marginVertical: 10, height: 700 }}>
                  ...
                  <View style={{ flex: 9, padding: 5, }}>
                    <TextInput 
                      ...
                      maxLength={50}
                      value={formCat}
                      right={<TextInput.Icon icon="chevron-down" color="#fff" onPress={() => setSelectMode("4")} />} 
