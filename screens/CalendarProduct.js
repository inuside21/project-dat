...
// Entry Point
// ===============================================
export default function CalendarProduct({ navigation, route }) {
  // Variable
  // ===========================
  ...
  function LoadSearch()
  {
    ...
    const urlReq = ConfigConnection.server + "api.php?mode=52";
    fetch(urlReq, {
      method: 'POST',
      headers: { 'Content-Type': 'multipart/form-data' },
      body: JSON.stringify({
        sid: searchData.season_id,
        mid: searchData.month_number,
      }),
    })
              ...
              <View style={styles.mainForm} >
                ...
                <Surface elevation={2} style={{ marginVertical: 10, height: 700 }}>
                  <View style={{ flex: 1, flexDirection: "row", justifyContent: "flex-start", marginVertical: 10, marginHorizontal: 10 }}>
                    ...
                    <Text style={{ fontWeight: "bold", fontSize: 20, marginLeft: 10 }}>{calName[searchData.month_number]}</Text>
                  </View>
                  ...
                  <View style={{ flex: 1, marginTop: -10, marginHorizontal: 15, justifyContent: "center", alignItems: "center", backgroundColor: ConfigColors.GREENCOLOR, borderRadius: 12 }}>
                    <Text style={{ fontWeight: "bold", color: "#fff" }}>Best seeds to plant in the month of {calName[searchData.month_number]}</Text>
                  </View>

                  <View style={{ flex: 13, flexDirection: "row", flexWrap: "wrap", justifyContent: "flex-start" }}>
                    { resultList && resultList.length > 0 &&
                        resultList.map(r => {
                          return (
                            <View style={{ backgroundColor: "#24AC64", width: 87, height: 100, marginVertical: 10, marginHorizontal: 15 }}>
                              <Image source={{ uri: ConfigConnection.image + r.product_img }} style={{ height: "80%", width: "100%" }} />
                              <Text style={{ color: "#f5f5f5", fontSize: 9, fontWeight: "bold", alignSelf: "center", paddingVertical: 3 }}>{ r.product_name }</Text>
                            </View>
                          )
                        })
                    }
