...
// Entry Point
// ===============================================
export default function EditTradingData({ navigation, route }) {
  // Variable
  // ===========================
  ...
  // Start
  {
    ...
    useEffect(() => {
      if (selectMode != "")
      {
        navigation.navigate("SelectCategory", { sdata: selectMode, slink: "EditTradingData" })
        setSelectMode("")
      }
    }, [selectMode])
    ...
    useEffect(() => {
      ...
      {
        // cat
        if (route.params?.mSelect == "0")
        {
          setFormCat(route.params.rSelect.dName);
        }

        // classification
        if (route.params?.mSelect == "1")
        {
  ...
  function LoadProduct()
  {
    ...
    const urlReq = ConfigConnection.server + "api.php?mode=22";
    fetch(urlReq, {
      method: 'POST',
  ...
  const onPressSubmit = () => {
    ...
    // send data
    {
      const urlReq = ConfigConnection.server + "api.php?mode=21";
      fetch(urlReq, {
        method: 'POST',
        headers: { 'Content-Type': 'multipart/form-data' },
        body: JSON.stringify({
  ...
  const OnPressDelete = () => {
    ...
    // send data
    {
      const urlReq = ConfigConnection.server + "api.php?mode=24";
      fetch(urlReq, {
        method: 'POST',
        headers: { 'Content-Type': 'multipart/form-data' },
        body: JSON.stringify({
                  ...
                  <View style={{ flex: 9, padding: 5, }}>
                    <ScrollView>
                      <TextInput 
                        ...
                        maxLength={50}
                        value={formCat}
                        right={<TextInput.Icon icon="chevron-down" color="#fff" onPress={() => setSelectMode("0")} />} 
