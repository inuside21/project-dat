...
// Entry Point
// ===============================================
export default function ViewSupport({ navigation, route }) {
  // Variable
  // ===========================
  const userData = route.params.data;
  const searchData = route.params.sdata;  // "Labor Support"
  const searchMode = route.params.smode;  // 43
  ...
  function LoadSearch()
  {
    ...
    .then((dat) => {
      if (dat.status == "ok")
      {
        setResultData(dat.data.cat);    // category result
        setResultMode(dat.data.smode);  // 42
        setResultList(dat.data.list);   // support list result
      }
    ...
    <Surface elevation={2} style={{ marginHorizontal: 10, marginVertical: 10, height: 75 }}>
      <View style={{ flex: 1, flexDirection: "row", marginVertical: 10, marginHorizontal: 10 }}>
        ...
        <View style={{ marginLeft: 10, justifyContent: "center", flexShrink: 1 }}>
          <Pressable onPress={() => navigation.navigate('ViewSupportData', { data: userData, sid: item.id, smode: resultMode })}>
    ...
    <Surface elevation={2} style={{ marginHorizontal: 10, marginVertical: 10, height: 150, width: '43%' }}>
      ...
      <Pressable onPress={() => navigation.navigate('ViewSupportData', { data: userData, sid: item.id, smode: resultMode })}>
        ...
        <View style={{ justifyContent: "center", marginHorizontal: 5 }}>
            <Text style={{ fontWeight: "bold", fontSize: 12 }}>{item.trading_name}</Text>
            <Text style={{ fontWeight: "bold", color: "#FF0000", fontSize: 8, marginTop: 10 }}>{searchData}</Text>
            <Text style={{ fontSize: 8 }}>{item.trading_location} - {item.user.user_fname}</Text>
        </View>
        </Pressable>
      </View>
    </Surface>
