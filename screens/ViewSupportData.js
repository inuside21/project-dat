...
// Entry Point
// ===============================================
export default function ViewSupportData({ navigation, route }) {
  // Variable
  // ===========================
  const userData = route.params.data;
  const searchId = route.params.sid;        // support_tbl id
  const searchMode = route.params.smode;    // 42
  ...
  const [isTxnLoaded, setIsTxnLoaded] = useState(false);
  ...
  function LoadSearchData()
  {
    ...
    .then((dat) => {
      if (dat.status == "ok")
      {
        setFormData(dat.data.info);   // support_tbl data
        setFormImage(dat.data.img);
        setSellerData(dat.data.user);

        if (dat.data.info.user_id != userData.id)
        {
          LoadTransaction(userData.id, dat.data.user.id, dat.data.type, dat.data.info.id);
        }
      }
  ...
  function LoadTransaction(tbuyer, tseller, titype, tiid)
  {
    const urlReq = ConfigConnection.server + "api.php?mode=msginit";
    fetch(urlReq, {
      method: 'POST',
      headers: { 'Content-Type': 'multipart/form-data' },
      body: JSON.stringify({
  ...
  const onPressMessage = () => {
    navigation.navigate('Message', { data: userData, txnid: txnData.id });  
  }
              ...
              <View style={styles.mainForm} >
                ...
                <Surface elevation={2} style={{ marginVertical: 10, height: 700 }}>
                  <View style={{ flex: 1, flexDirection: "row", justifyContent: "flex-start", marginVertical: 10, marginHorizontal: 10 }}>
                    ...
                    <Text style={{ fontWeight: "bold", fontSize: 20, marginLeft: 10 }}>Service Details</Text>
                  </View>
                    ...
                    isTxnLoaded &&
                      <View style={{ flex: 1, marginHorizontal: 100 }}>
                        { formData.user_id != userData.id &&
                            <Button mode="contained" icon="email-outline" buttonColor={ConfigColors.GREENCOLOR} contentStyle={{ height: 40 }} style={styles.formButton} onPress={onPressMessage}>
                              Message
                            </Button>
                        }
                      </View>
