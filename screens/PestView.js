...
// Entry Point
// ===============================================
export default function PestView({ navigation, route }) {
  // Variable
  // ===========================
  ...
  // Start
  {
    ...
    useEffect(() => {
      if (route.params?.stxt)
      {
        setTextSearch(route.params.stxt);
      }
    }, [route.params?.stxt])
  }
  ...
  function LoadSearch()
  {
    ...
    const urlReq = ConfigConnection.server + "api.php?mode=60";
    fetch(urlReq, {
      method: 'POST',
  ...
  const ResultItem = ({ item, index }) => (
    <>
        <View style={{ backgroundColor: "#24AC64", width: 87, height: 100, marginVertical: 10, marginHorizontal: 15 }}>
          <Pressable onPress={() => navigation.navigate('PestViewData', { data: userData, sdata: item })}>
            <Image source={{ uri: ConfigConnection.image + item.pest_img }} style={{ height: "80%", width: "100%" }} />
            <Text style={{ color: "#f5f5f5", fontSize: 9, fontWeight: "bold", alignSelf: "center", paddingVertical: 3 }}>{ item.pest_name }</Text>
          </Pressable>
        </View>
    </>
