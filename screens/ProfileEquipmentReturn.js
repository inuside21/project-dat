...
// Entry Point
// ===============================================
export default function ProfileEquipmentReturn({ navigation, route }) {
  // Variable
  // ===========================
  ...
  function LoadProductList()
  {
    ...
    const urlReq = ConfigConnection.server + "api.php?mode=81a";
    fetch(urlReq, {
      method: 'POST',
      ...
      <Surface elevation={2} style={{ marginHorizontal: 10, marginVertical: 10, height: 75 }}>
        <View style={{ flex: 1, flexDirection: "row", marginVertical: 10, marginHorizontal: 10 }}>
          ...
          <View style={{ marginLeft: 10, justifyContent: "center", flexShrink: 1 }}>
            <Pressable onPress={() => navigation.navigate('ProfileEquipmentReturnData', { data: userData, smode: item.search, sid: item.id, tdata: item.transact, sdata: item.user })}>
