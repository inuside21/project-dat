...
// Entry Point
// ===============================================
export default function TradingAdd({ navigation, route }) {
  // Variable
  // ===========================
  ...
  // Start
  {
    ...
    useEffect(() => {
      if (selectMode != "")
      {
        navigation.navigate("SelectCategory", { sdata: selectMode, slink: "TradingAdd" })
        setSelectMode("")
      }
    }, [selectMode])
  ...
  const onPressSubmit = () => {
    ...
    // send data
    {
      const urlReq = ConfigConnection.server + "api.php?mode=20";
      fetch(urlReq, {
        method: 'POST',
